...
func (r *UpstreamRouter) HandleUpstream(ctx context.Context, userID kernel.UserID, payload []byte) []byte {
	...
	kafkaTopic, ok := r.allowedActions[string(env.Action)]
	if !ok {
	...
	// 将 Envelope 装裱为一个领域事件发送给 Kafka，使用映射后的内部真实 Topic
	ev := event.NewStandardEvent(kernel.ID(userID), kafkaTopic, env.Payload, r.idGenerator)
	...
	if err != nil {
		zap.L().Error("gateway: sync publish to kafka failed", zap.Error(err), zap.String("action", string(env.Action)))
