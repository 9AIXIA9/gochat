...
func (repo *EventRepository) UnpublishedList(ctx context.Context) ([]event.Event, error) {
	...
	if err := repo.unitOfWork.DB(ctx).WithContext(ctx).Find(&models).Error; err != nil {
...
func (repo *EventRepository) MarkPublished(ctx context.Context, ID event.ID) error {
	return gormutils.TranslateError(repo.unitOfWork.DB(ctx).WithContext(ctx).Delete(&model.Event{}, "id = ?", ID).Error)
}
