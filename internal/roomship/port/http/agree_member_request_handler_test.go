...
type fakeAgreeMemberRequestUseCase struct {
	exec func(ctx context.Context, in *roomshipApplication.AgreeMemberRequestInput) (*kernel.NoOutput, error)
}

func (f fakeAgreeMemberRequestUseCase) Execute(ctx context.Context, in *roomshipApplication.AgreeMemberRequestInput) (*kernel.NoOutput, error) {
	return f.exec(ctx, in)
}

func TestAgreeMemberRequestRequest_Bind(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		want      *roomshipHTTP.AgreeMemberRequestRequest
	}{
		{
			name:      "success",
			userID:    fixedUserID,
			requestID: fixedRequestID,
			want:      &roomshipHTTP.AgreeMemberRequestRequest{UserID: fixedUserID, RequestID: fixedRequestID},
		},
		{
			name:      "missing request id",
			userID:    fixedUserID,
			requestID: "",
			want:      &roomshipHTTP.AgreeMemberRequestRequest{UserID: fixedUserID, RequestID: ""},
		},
	}
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			req, err := http.NewRequest(http.MethodPut, "/rooms/requests/request-1/agree", nil)
			...
			request := &roomshipHTTP.AgreeMemberRequestRequest{}
...
func TestNewAgreeMemberRequestHandler(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		path           string
		expectValidate bool
		useCase        fakeAgreeMemberRequestUseCase
		expectResponse *api.Response
	}{
		{
			name:           "success maps request to input",
			injectUserID:   fixedUserID,
			path:           "/rooms/requests/request-1/agree",
			expectValidate: true,
			useCase: fakeAgreeMemberRequestUseCase{exec: func(_ context.Context, in *roomshipApplication.AgreeMemberRequestInput) (*kernel.NoOutput, error) {
		...
		},
		{
			...
			path:           "/rooms/requests/request-1/agree",
			expectValidate: true,
			useCase: fakeAgreeMemberRequestUseCase{exec: func(_ context.Context, _ *roomshipApplication.AgreeMemberRequestInput) (*kernel.NoOutput, error) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			router.PUT("/rooms/requests/:request_id/agree", roomshipHTTP.NewAgreeMemberRequestHandler(tt.useCase, validator))
