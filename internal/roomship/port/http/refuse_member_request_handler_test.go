...
type fakeRefuseMemberRequestUseCase struct {
	exec func(ctx context.Context, in *roomshipApplication.RefuseMemberRequestInput) (*kernel.NoOutput, error)
}

func (f fakeRefuseMemberRequestUseCase) Execute(ctx context.Context, in *roomshipApplication.RefuseMemberRequestInput) (*kernel.NoOutput, error) {
	return f.exec(ctx, in)
}

func TestRefuseMemberRequestRequest_Bind(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		want      *roomshipHTTP.RefuseMemberRequestRequest
	}{
		{
			name:      "success",
			userID:    fixedUserID,
			requestID: fixedRequestID,
			want:      &roomshipHTTP.RefuseMemberRequestRequest{UserID: fixedUserID, RequestID: fixedRequestID},
		},
		{
			name:      "missing request id",
			userID:    fixedUserID,
			requestID: "",
			want:      &roomshipHTTP.RefuseMemberRequestRequest{UserID: fixedUserID, RequestID: ""},
		},
	}
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			req, err := http.NewRequest(http.MethodPut, "/rooms/requests/request-1/refuse", nil)
			...
			request := &roomshipHTTP.RefuseMemberRequestRequest{}
...
func TestNewRefuseMemberRequestHandler(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		path           string
		expectValidate bool
		useCase        fakeRefuseMemberRequestUseCase
		expectResponse *api.Response
	}{
		{
			name:           "success maps request to input",
			injectUserID:   fixedUserID,
			path:           "/rooms/requests/request-1/refuse",
			expectValidate: true,
			useCase: fakeRefuseMemberRequestUseCase{exec: func(_ context.Context, in *roomshipApplication.RefuseMemberRequestInput) (*kernel.NoOutput, error) {
		...
		},
		{
			...
			path:           "/rooms/requests/request-1/refuse",
			expectValidate: true,
			useCase: fakeRefuseMemberRequestUseCase{exec: func(_ context.Context, _ *roomshipApplication.RefuseMemberRequestInput) (*kernel.NoOutput, error) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			router.PUT("/rooms/requests/:request_id/refuse", roomshipHTTP.NewRefuseMemberRequestHandler(tt.useCase, validator))
