...
type Repository[
	...
	client                     *redis.Client
	redisGenericModelConverter kernel.GenericModelConverter[*RedisModel, *DomainModel]
	KeyPrefix                  string
}
...
func NewRepository[
	RedisModel any,
	DomainModel any,
](client *redis.Client, converter kernel.GenericModelConverter[*RedisModel, *DomainModel], keyPrefix string) *Repository[RedisModel, DomainModel] {
...
func (r *Repository[RedisModel, DomainModel]) Save(ctx context.Context, key string, model *DomainModel, ttl ...time.Duration) error {
	...
	redisModel, err := r.redisGenericModelConverter.ToModel(model)
	if err != nil {
		return err
	}
...
func (r *Repository[RedisModel, DomainModel]) Find(ctx context.Context, key string) (*DomainModel, error) {
	...
	return r.redisGenericModelConverter.ToDomain(redisModel)
}

func (r *Repository[RedisModel, DomainModel]) FindMany(ctx context.Context, keys []string) ([]*DomainModel, error) {
	if len(keys) == 0 {
		return nil, nil
	}
	...
	for _, v := range vals {
		...
		domainModel, err := r.redisGenericModelConverter.ToDomain(redisModel)
		if err != nil {
			return nil, err
		}
