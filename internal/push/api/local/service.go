...
// PushService 是 PushService 在单体进程内环境的默认实现。
type PushService struct {
	manager *core.Manager
}

// NewPushService 创建本地推送服务
func NewPushService(manager *core.Manager) contract.PushService {
	return &PushService{
		manager: manager,
	}
}
...
func (s *PushService) PushToUser(ctx context.Context, userID string, payload []byte) error {
...
func (s *PushService) Broadcast(ctx context.Context, userIDs []string, payload []byte) error {
	if timeout.CheckCtxTimeout(ctx) {
		return myErrors.ErrTimeout
	}
