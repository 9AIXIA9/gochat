...
type fakeSendPrivateMessageUseCase struct {
	exec func(ctx context.Context, in *chatApplication.SendPrivateMessageInput) (*kernel.NoOutput, error)
}

func (f fakeSendPrivateMessageUseCase) Execute(ctx context.Context, in *chatApplication.SendPrivateMessageInput) (*kernel.NoOutput, error) {
	return f.exec(ctx, in)
}

func TestSendPrivateMessageRequest_Bind(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		body      string
		expectErr bool
		want      *chatHTTP.SendPrivateMessageRequest
	}{
		{
			name:      "success",
			userID:    fixedSenderID,
			body:      `{"recipient_id":"user-2","content":"hello"}`,
			expectErr: false,
			want: &chatHTTP.SendPrivateMessageRequest{
				SenderID:    fixedSenderID,
				RecipientID: kernel.UserID("user-2"),
				Content:     "hello",
			},
		},
		{
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			req, err := http.NewRequest(http.MethodPost, "/chats/private-messages", bytes.NewBufferString(tt.body))
			require.NoError(t, err)
			req = req.WithContext(ctxutil.WithUserID(req.Context(), tt.userID))
			req.Header.Set("Content-Type", "application/json")
			ginContext.Request = req

			request := &chatHTTP.SendPrivateMessageRequest{}
...
func TestNewSendPrivateMessageHandler(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		body           string
		expectValidate bool
		useCase        fakeSendPrivateMessageUseCase
	...
	}{
		{
			...
			body:           body,
			expectValidate: true,
			useCase: fakeSendPrivateMessageUseCase{exec: func(_ context.Context, in *chatApplication.SendPrivateMessageInput) (*kernel.NoOutput, error) {
				assert.Equal(t, fixedSenderID, in.SenderID)
				assert.Equal(t, kernel.UserID("user-2"), in.RecipientID)
		...
		},
		{
			...
			useCase: fakeSendPrivateMessageUseCase{exec: func(_ context.Context, _ *chatApplication.SendPrivateMessageInput) (*kernel.NoOutput, error) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			router.POST("/chats/private-messages", chatHTTP.NewSendPrivateMessageHandler(tt.useCase, validator))
